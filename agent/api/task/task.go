...
func (task *Task) dockerExposedPorts(container *apicontainer.Container) (dockerExposedPorts nat.PortSet, err error) {
	...
	for _, portBinding := range containerToCheck.Ports {
		...
		if portBinding.ContainerPort != nil {
			...
			// we supply containerPortRange here in case we did not assign a contiguous host port range and ask docker to do so
	...
	for _, portBinding := range containerToCheck.Ports {
		...
		if portBinding.ContainerPort != nil {
			...
			if err != nil {
				...
				// nat.ParsePortSpec returns a list of mappings in a format that docker likes
				...
				for _, pm := range mappings {
					dockerPortMap[pm.Port] = append(dockerPortMap[pm.Port], pm.Binding)
				}
