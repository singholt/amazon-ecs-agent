...
// image level. Presence of any of these keys in the image's config suppresses
// agent-side default injection at container create time. Add keys here to
// extend tracking to new vars.
...
// given image env list (image config Env, formatted as "KEY=value" or "KEY").
// A managed key with an empty value is still recorded; an info log is emitted
// because empty managed env values in an image are unusual.
...
type ImageState struct {
	...
	PullSucceeded bool
	// ManagedEnvKeys is the subset of agent-managed env var keys (see managedEnvKeys)
	// that are present in the image config. Populated at image-inspection time and
	// persisted with the rest of ImageState so that the agent does not need to
	// re-inspect the image on restart.
